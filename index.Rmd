---
title: "Considering the Ripple Effects of Climate Change On Societal Public Health"
...
PM25_1990_leaflet <- leaflet(data = world_leaflet)%>%
  ...
  addLegend(pal = pal1990, values = ~Y1990, 
            opacity = 0.7, 
            title = "Exposure to PM2.5 in 1990 (µg / m<sup>3</sup>)",
...
PM25_2017_leaflet <- leaflet(data = world_leaflet)%>%
  ...
  addLegend(pal = pal2017, values = ~Y2017, 
            opacity = 0.7, 
            title = "Exposure to PM2.5 in 2017 (µg / m<sup>3</sup>)",
            position = "bottomright")
...
PM25_change_leaflet <- leaflet(data = world_leaflet)%>%
  ...
  addLegend(pal = palchange, values = ~change, 
            opacity = 0.7, 
            title = "Change in Exposure to PM 2.5 <br> from 1990 to 2017 (µg / m<sup>3</sup>)",
            position = "bottomright")
```
...
DALY_change_leaflet <- leaflet(data = world_leaflet_DALY)%>%
  ...
  addPolygons(fillColor = ~palchange2(change),
              ...
              highlight = highlightOptions(weight = 2,
                                           ...
                                           fillOpacity = 0.7,
                                            bringToFront = TRUE),
              label = sprintf("<strong>%s</strong><br/>%g years per 1K inhabitants",
              ...
              popup = paste0("Country: ", world_leaflet_DALY$country, "<br>",
                             ...
                             " years per 1K inhabitants <br>")) %>%
   addLegend(pal = palchange2, values = ~change, 
            opacity = 0.7, 
             title = "Change in DALY",
             position = "bottomright")
```
...
g <- ggplot(data_joined, aes(PM25exposure , DALY, color = country))+
  ...
  labs(title = 'Year: {round(frame_time, 0)}', x = 'Expusure to PM 2.5 (µg / m<sup>3</sup>)', y = 'DALY per 1K Inhabitants (years)')+
...
gdaly <- ggplotly(ggplot(data = data_joined, mapping = aes(x = year, y = DALY, color = country)) +
  ...
  scale_y_continuous(trans = "log10")+
    labs(
      title = "Disability-Adjusted Life (DALY) Years per 1K inhabitants",
...
When looking at the original dataset, we can see that without looking at specific countries, there is a trend of decreasing infant and maternal mortality (compare country mortality rates in 1980 to mortality rates in 2018). At first this was surprising, as we expected developed/developing countries to show stagnancy or worsening numbers due to the prevalence and increasing intensity of climate disasters. However, it should be noted that even in the 1980s, the available medical technology was vastly inferior to that we have today, in any country. Thus, the overall decreasing trend is due to improvements in surgical technology, medical infrastructure, hygiene, etc. that has come in the 21st century.
...
# Climate Disaster Frequency and Displacement of People (LK)
...
One limitation that we wanted to remind you of was that this dataset did not record every single storm, drought, and flood that happened for the past few decades. Rather, it only recorded the natural disasters that affected humans beyond a certain extent. Therefore, the increased frequency of natural disasters shown in the first plot should be understood as a proxy, rather than a direct measurement of the frequency of natural disasters. We cannot rule out the possibility that the dataset may have failed to accurately document the natural disasters of earlier years. 

Therefore, having a more extensive dataset certainly would have enhanced the quality of our analysis, and allowed us to compare how different countries were differentially affected by different types of natural disasters. For example, some regions may be more likely to experience droughts than floods, and vice versa. Or, region A and region B may be equally likely to experience droughts, but one may be more affected than the other. Without a more extensive dataset, it is impossible to establish a) that natural disasters truly became more frequent over time; and b) that increased frequency of disasters is correlated to intensified consequences.

Nevertheless, the choropleth (based on a somewhat incomplete data) clearly showed that some countries suffered more than others. Why might this be the case? Again, A storm may barely affect developed countries where houses and buildings were designed to withstand natural disasters. In this case the storm will not be recorded in this dataset. However, the same storm may wreak havoc in developing countries that lack infrastructures to predict storms, prepare for the storms, and take necessary protective measures. People in such regions would be more likely to lose houses, clean water sources, and access to public health services, when they are already more likely to be killed and injured without the appropriate protection. Therefore, we can conclude that **although there _might_ be no true between-region differences in the frequency of natural disasters, it is highly likely that some developing regions suffer/are affected disproportionately more from natural disasters than developed regions**.
