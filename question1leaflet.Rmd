...
bins3 <- c(-900, -15, -10, -5, 0, 5, 900)
bins4 <- c(0, 5, 10, 15, 20, Inf)
pal1990_3 <- colorBin("YlGnBu", domain = world_leaflet_DALY$Y1990, bins = bins4)
pal2017_3 <- colorBin("YlGnBu", domain = world_leaflet_DALY$Y2017, bins = bins4)
palchange3 <- colorNumeric("YlGnBu", domain = world_leaflet_DALY$change)
...
DALY_change_leaflet <- leaflet(data = world_leaflet_DALY)%>%
  addTiles()%>%
  setView(0, 0, zoom = 1.5)%>%
  addPolygons(fillColor = ~palchange3(change),
              weight = 1,
              opacity = 1,
              color = "white",
              dashArray = "3",
